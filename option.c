...
OPT_s_datum_t* OPT_f_new( FLA_s_data_t* ps_flag_data )
{
	...
	const FLA_s_datum_t* ps_flag_datum_iterations = FLA_f_find_by_key_raw( ps_flag_data, -1, "iterations" );
	...
	const bool b_iterations = ( ps_flag_datum_iterations != NULL && ps_flag_datum_iterations->ps_value != NULL );
	const unsigned long n_iterations = ( ( b_iterations ) ? strtoul( ps_flag_datum_iterations->ps_value->pc_value, NULL, 10 ) : 0 );

	const bool b_daemon = (
		FLA_f_contains_by_key_raw( ps_flag_data, 1, "d" ) ||
		FLA_f_contains_by_key_raw( ps_flag_data, -1, "daemon" )
	);
	...
	ps_datum->n_iterations = n_iterations;
...
static void f_handle_command( const OPT_s_datum_t* ps_option_datum )
{
	...
	if ( ps_option_datum->ps_command != NULL )
		if ( ps_option_datum->n_iterations == 0 )
			for ( ;; )
				system( ps_option_datum->ps_command_with_args->pc_value );
		else
			for ( unsigned long n_i = ps_option_datum->n_iterations; n_i > 0; --n_i )
				system( ps_option_datum->ps_command_with_args->pc_value );
}
